...
test('registered users do not get public messenger access without an approved quest', async () => {
  ...
  try {
    const registerResult = await requestJson(baseUrl, '/api/register', {
      ...
      body: JSON.stringify({ username: 'publicuser', password: 'public123' })
    });
    assert.equal(registerResult.response.status, 200);
    assert.equal(registerResult.data.success, true);
    assert.equal(registerResult.data.user.access, false);

    const meResult = await requestJson(baseUrl, '/api/user', {
      headers: { Authorization: `Bearer ${registerResult.data.token}` }
    });
    assert.equal(meResult.response.status, 200);
    assert.equal(meResult.data.user.access, false);
  } finally {
